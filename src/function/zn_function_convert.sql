...
----
DROP FUNCTION IF EXISTS zn_plugin_admin_role_owners;
create function zn_plugin_admin_role_owners(_ids varchar(250))
returns varchar(1000)
begin
	declare _seeds varchar(1000) default ',';
	declare _seed varchar(100) default '';
	...
	set _len = char_length(_ids);
		...
		select owners into _seed from zn_plugin_admin_role where convert(id, char(10))=IFNULL(_id, '');

		if char_length(_seed)>=1 then
			set _seeds = concat(_seeds, _seed);
			set _seeds = replace(_seeds, ',,', ',');
		end if;
		end while;
...
----
DROP FUNCTION IF EXISTS zn_plugin_admin_user_role_owners;
create function zn_plugin_admin_user_role_owners(_userid int(11))
returns varchar(1000)
begin
	declare _ids varchar(1000) default ',';
	declare _seeds varchar(1000) default ',';
	declare _seed varchar(100) default '';
	declare _id varchar(10);
	declare _split int;
	declare _len int;
	select role_ids into _ids from zn_plugin_admin_user where id=_userid;
	set _len = char_length(_ids);
