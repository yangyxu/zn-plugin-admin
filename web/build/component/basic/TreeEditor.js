...
module.exports = React.createClass({
	...
	getDefaultProps: function getDefaultProps() {
		return {
			...
			pid: 0,
			fields: [{ title: '显示值', name: 'zn_title', type: 'Textarea' }]
		};
	},
	getInitialState: function getInitialState() {
		return {
			...
			mainToolbarItems: [{ tooltip: '添加主项', value: 'addMain', icon: 'fa-plus-square' }],
			subToolbarItems: [{ tooltip: '添加子项', value: 'addChild', icon: 'fa-plus' }, { tooltip: '删除当前项', value: 'delete', icon: 'fa-remove' }, { tooltip: '编辑当前项', value: 'edit', icon: 'fa-edit' }, { tooltip: '上移当前项', value: 'up', icon: 'fa-angle-up' }, { tooltip: '下移当前项', value: 'down', icon: 'fa-angle-down' }]
		};
	},
	...
	__itemContentRender: function __itemContentRender(data, treeColumn) {
		...
		if (_result) {
			...
			return React.createElement(
				...
				data.icon && React.createElement(zn.react.Icon, { icon: data.icon }),
				React.createElement(
					'span',
	...
	render: function render() {
		return React.createElement(
			...
			this.props.editable && React.createElement(
				...
				this.state.mainToolbarItems.map(function (item) {
					...
					return React.createElement('i', { onClick: function onClick() {
							return _this.__onToolbarClick(item);
						}, 'data-tooltip': item.tooltip, className: "fa " + item.icon });
				}.bind(this)),
				React.createElement('i', { className: 'separator' }),
			...
			React.createElement(zn.react.Tree, _extends({}, this.props, {
