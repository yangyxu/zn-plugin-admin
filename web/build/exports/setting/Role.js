	...
	__renderBody: function __renderBody() {
		switch (this.state.currIndex) {
			case 0:
				...
				return React.createElement(ModelEditor, _extends({}, this.props, { fields: _fields }));
			case 1:
				return React.createElement(UsersForRoles, { roleId: this.props.data.value });
		}
	},
	render: function render() {
		...
		return React.createElement(
			...
			React.createElement(zn.react.ListView, {
				...
				data: [{ index: 0, text: '基本信息', icon: 'fa-list-alt' }, { index: 1, text: '用户列表', icon: 'fa-users' }] }),
			React.createElement(
				'div',
...
module.exports = React.createClass({
	...
	getDefaultProps: function getDefaultProps() {
		return {
			...
			title: '部门及角色管理',
			menuId: 0,
			fields: [{ title: '名称', type: 'Input', name: 'zn_title' }, { title: '类型', type: 'Radio', name: 'type', value: 0,
				data: [{ text: '分类', value: 0 }, { text: '部门', value: 1 }, { text: '角色', value: 2 }]
			}, { title: '扩展', type: 'Textarea', name: 'zn_tree_extend' }, { title: '说明', type: 'Textarea', name: 'zn_note' }]
		};
	},
	__rightRender: function __rightRender(data) {
		...
		return React.createElement(Info, _extends({}, this.props, { data: data }));
	},
