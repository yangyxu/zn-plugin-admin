...
var Config = React.createClass({
		...
		if (this.props.method) {
			...
			if (_items.length) {
				return React.createElement(
					zn.react.Group,
					{ title: '\u8BF7\u6C42\u53C2\u6570' },
					React.createElement(zn.react.Form, { items: _items, buttons: [{ text: '发送请求', type: 'submit', status: 'primary' }], onSubmitBefore: this.__onFormSubmitBefore })
				);
			} else {
				return React.createElement(zn.react.Button, { onClick: function onClick() {
						return _this.__submit();
					}, float: 'right', text: '\u53D1\u9001\u8BF7\u6C42' });
			}
		} else {
			return null;
		}
	...
	__onRemove: function __onRemove() {
		zn.confirm('确定删除该参数？', '提示', function () {
			...
			zn.http.post('/zn.plugin.admin/config/deleteById', { id: this.props.id }).then(function (data) {
				if (data.status == 200) {
			...
			React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						'div',
						{ className: 'head-left' },
						this.props.input_type && React.createElement(
							'span',
							{ className: 'method' },
					...
					React.createElement(
						...
						React.createElement(
							'span',
							{ onClick: this.__onSave, className: 'method' },
							React.createElement('i', { className: 'fa fa-save zr-padding-3' }),
							'\u4FDD\u5B58'
						),
						React.createElement(
							'span',
							{ onClick: this.__onEdit, className: 'method' },
							React.createElement('i', { className: 'fa fa-edit zr-padding-3' }),
							'\u7F16\u8F91'
						),
						React.createElement('i', { onClick: this.__onRemove, className: 'fa fa-remove zr-padding-3' })
					)
				),
				React.createElement(
					'div',
					{ className: 'i-body' },
					React.createElement(zn.react.FormItem, { type: _Input, title: this.props._title, value: this.props._value })
				)
			)
		);
	}
...
module.exports = React.createClass({
	...
	getInitialState: function getInitialState() {
		return {
			...
			key: null,
			configs: [],
			toolbarItems: [{ text: '配置查询', name: 'config.list', icon: 'fa-list', style: { marginRight: 5 } }, { text: '添加', name: 'config.add', icon: 'fa-plus', style: { marginRight: 5 } }, { text: '删除', name: 'config.delete', status: 'danger', icon: 'fa-trash', style: { marginRight: 5 } }]
		};
	},
	...
	__onAppClick: function __onAppClick(key) {
		...
		zn.http.get("/zn.plugin.admin/config/selectBy_id", { _id: key.value }).then(function (data) {
			if (data.status == 200) {
				this.setState({
					key: key,
					configs: data.result
	...
	__renderConfigKey: function __renderConfigKey() {
		return React.createElement(
			...
			this.state.configKeys.map(function (key, index) {
				...
				return React.createElement(
					'li',
					{ key: index, className: "app " + (this.state.key == key ? 'curr' : ''), onClick: function onClick() {
	...
	__addArgument: function __addArgument() {
		...
		zn.dialog({
			...
			content: React.createElement(zn.react.Form, {
				...
				items: [{ title: '_title', name: '_title', type: 'Input' }, { title: '_key', name: '_key', type: 'Input' }, { title: '_value', name: '_value', type: 'Input' }, { title: 'ref_id', name: 'ref_id', type: 'Input' }, { title: 'var_id', name: 'var_id', type: 'Input' }, { title: 'InputType', name: 'input_type', type: 'Menu', data: ['Input', 'ImageUploader', 'FileUploader', 'Textarea', 'RichEditor'] }, { title: 'DataType', name: 'data_type', type: 'Input' }, { title: 'RichValue', name: '_rich_value', type: 'Textarea' }] })
		});
	},
	__renderConfigs: function __renderConfigs() {
		return React.createElement(
			...
			this.state.key && React.createElement(
				'div',
				{ style: { padding: 5 } },
				React.createElement(zn.react.Button, { onClick: this.__addArgument, text: '\u6DFB\u52A0\u53C2\u6570', icon: 'fa-plus' })
			),
			React.createElement(
				'ul',
				{ className: 'apis' },
				Object.keys(this.state.configs).map(function (key, index) {
					...
					return React.createElement(Config, _extends({ key: index }, this.state.configs[key], { onRemoveSuccess: function onRemoveSuccess() {
	...
	render: function render() {
		return React.createElement(
			zn.react.Page,
			{ className: 'zn-plugin-admin-setting-config-manager', title: '\u914D\u7F6E\u7BA1\u7406', toolbarItems: this.state.toolbarItems, onToolbarClick: this.__onToolbarClick },
			React.createElement(
				'div',
