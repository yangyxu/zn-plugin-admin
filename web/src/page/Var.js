...
module.exports = React.createClass({
	getDefaultProps: function () {
		return {
			...
			fields: [
				...
				{ title: '图片', type: 'ImageUploader', name: 'img', action: '/znadmin/uploadFiles' },
		...
		return <div>
			<RightsSetting model={this.props.model} id={treeModel.state.currItem?treeModel.state.currItem.props.data.id:null}  />
			{
				...
				<UI.Form
					...
					merge="data"
					value={Store.post('/znadmin/model/selectOne', {model: this.props.model, where: { id: _id }})}
					syncSubmit={false}
	...
	__itemContentRender: function (item){
		...
		switch (_data.type) {
			case 1:
				return <span><i title="这是操作按钮" className='fa fa-hand-o-up' style={{margin: 5, color: '#0B72A5'}} /><i className={'fa ' + _data.icon} style={{marginRight:5}} />{_data.id+'、'+_data.title}</span>;
			case 2:
				return <span><i title="这是静态常量" className='fa fa-text-width' style={{margin: 5,color: '#d9534f'}} /><i className={'fa ' + _data.icon} style={{marginRight:5}} />{_data.id+'、'+_data.title}</span>;
			case 3:
				return <span><i title="这是标签类别" className='fa fa-tag' style={{margin: 5}} /><i className={'fa ' + _data.icon} style={{marginRight:5}} />{_data.id+'、'+_data.title}</span>;
		}
	},
	render:function(){
		return (
